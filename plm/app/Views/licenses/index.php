...
<div class="card">
    <div class="card-body">
        <form class="data-toolbar" method="get" action="<?= url('licenses') ?>">
            <input type="search" name="q" class="form-control" placeholder="<?= e(__('Search number / key / customer…')) ?>" value="<?= e($filters['term']) ?>">
            <select name="status" class="form-select">
                <?php foreach ($statuses as $k => $v): ?><option value="<?= $k ?>" <?= $filters['status'] === $k ? 'selected' : '' ?>><?= e(__($v)) ?></option><?php endforeach; ?>
            </select>
            <select name="type" class="form-select">
                <?php foreach ($types as $k => $v): ?><option value="<?= $k ?>" <?= $filters['type'] === $k ? 'selected' : '' ?>><?= e(__($v)) ?></option><?php endforeach; ?>
            </select>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead><tr><th><?= e(__('Number')) ?></th><th><?= e(__('Customer')) ?></th><th><?= e(__('Product')) ?></th><th><?= e(__('Type')) ?></th><th><?= e(__('Issued')) ?></th><th><?= e(__('Expiry')) ?></th><th><?= e(__('Status')) ?></th><th class="text-end"><?= e(__('Actions')) ?></th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-light text-dark border"><?= e(__(ucwords(str_replace('_', ' ', $l['type'])))) ?></span></td>
                        ...
                        <td class="text-end">
                            <a href="<?= url('licenses/' . $l['id']) ?>" class="btn btn-sm btn-outline-secondary"><?= e(__('View')) ?></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                    <?php if ($result['data'] === []): ?><tr><td colspan="8" class="empty-state"><?= e(__('No licenses found.')) ?></td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
